...
def print_difs(seq, target_seq, color='light_grey'):
    s_a, s_t = get_ali([seq, target_seq])
    diffs = np.array(list(s_a)) != np.array(list(s_t))
    color_seq = ''.join([colored(s, 'red' if d else color, attrs=['bold'] if d else []) for s, d in zip(s_a, diffs)])
    print(color_seq)
...
def is_alph(seqs):
    s_s = remove_spaces(seqs)
    return [len(s) for s in s_s] == get_ohe(s_s, alphabet_name='prot').sum(-1).sum(-1)
...
def get_lens(seqs_ohe):
    """ Get lengths of OHE sequences - numpy arrays. """
    return seqs_ohe.sum(axis=-1).sum(axis=-1)


################ ali dists #################

def seqs_to_fasta_text(seqs):
    for i, seq in enumerate(seqs):
        print(f'>{i}\n{seq}')


def seqs_to_fasta(seqs, fname):
...
def get_ali_dist(seqs, cut_first=0, clustalo_path='/root/miniconda3/envs/py312/bin/clustalo'):
    ...
    len_ali = len(seqs_ali[0])
    return ham_dist, len_ali
...
pad_seq_len = lambda seqs, pad_to_len: pad_axis(seqs, pad_to_len, -2, 0)
pad_seq_len_torch = lambda seqs, pad_to_len: pad_axis(seqs, pad_to_len,
                                                      -2, 0, use_torch=True)
pad_num_seqs = lambda seqs, pad_to_len: pad_axis(seqs, pad_to_len, -3, np.nan)
